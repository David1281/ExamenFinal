...
export class LexerDL extends Lexer{
    ...
    recognizeToken(accepted: boolean, analyzedString: string, acceptingState: number): Token{
        ...
        // Integer literal
        if(acceptingState == StatesDL.Entero || acceptingState == StatesDL.DecimalFinal){
            token = new Token(TokenTypes.IntegerLiteralC, TokenTypes[TokenTypes.IntegerLiteralC], analyzedString, this.line, this.column);            
            return token;
        }

        if(acceptingState == StatesDL.DecimalFinal){
            token = new Token(TokenTypes.IntegerLiteralC, TokenTypes[TokenTypes.IntegerLiteralC], analyzedString, this.line, this.column);            
            return token;
        }
        ...
        if(tokenType == null || tokenType == undefined){
            token = new Token(TokenTypes.IdentifierC, TokenTypes[TokenTypes.IdentifierC], analyzedString, this.line, this.column);                
        }else{
